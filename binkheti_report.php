...
$sql = "SELECT bfd.*, rl.location AS recording_location, SEC_TO_TIME(vl.length_in_sec) AS duration,
DATE(vl2.call_date) AS call_date, 
TIME(vl2.call_date) AS call_time
FROM binkheti_form_data bfd
INNER JOIN recording_log rl ON bfd.rec_id = rl.recording_id
INNER JOIN vicidial_log vl ON bfd.phonenumber = vl.phone_number
INNER JOIN vicidial_log vl2 ON bfd.lead_id = vl2.lead_id
...
AND (
    `binkheti_application_2` LIKE '%$answerFilter%' OR `binkheti_application_2` LIKE '%$answerFilter%'
) 
AND (
    CONCAT_WS(' ', `applicant_name`, `binkheti_relativename`, `applicant_location`, `applicant_taluko`, `applicant_jilla`, `star_rating`, 
    `binkheti_application_2`, `binkheti_application_3`, `binkheti_application_3_1`, `binkheti_difficulty_4`, `binkheti_multiple_selection_4`, 
    `binkheti_application_5`, `binkheti_application_reason_5`, `binkheti_application_response_6`, `binkheti_office_selection_6`, 
    `nikal_mangani`, `nikal_mangani_by_7`, `binkheti_difficulty_8`, `binkheti_application_8`, `phone_verification_8_1`, 
    `phone_verification_option_8_1`, `suggestion`, `applicant_name`, `applicant_location`, `applicant_taluko`, `applicant_jilla`)
    LIKE '%$wildcardFilter%'
)
GROUP BY bfd.id DESC";
...
if (isset($_GET['export']) && $_GET['export'] == 'xlsx') {
    ...
    if ($result->num_rows > 0) {
        ...
        $headers = [
            ...
            'Disposal Date',
            'Applicant Name',
            `binkheti_relativename`,
            'Applicant Location',
            'Applicant Taluko',
            'Applicant Jilla',
            'binkheti Application 2',
            'binkheti Application 3',
            'binkheti Application 3.1',
            'binkheti Difficulty 4',
            'binkheti Multiple Selection 4',
            'binkheti Application 5',
            'binkheti Application Reason 5',
            'binkheti Application Response 6',
            'binkheti Office Selection 6',
            'Nikal Mangani',
            'Nikal Mangani By 7',
            'binkheti Difficulty 8',
            'binkheti Application 8',
            'Phone Verification 8.1',
            'Phone Verification Option 8.1',
            'satisfaction',
            'Star Rating',
            'Additional Comments'
        ];
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_relativename']);
            $col++;
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_2']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_3']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_3_1']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_difficulty_4']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_multiple_selection_4']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_5']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_reason_5']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_response_6']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_office_selection_6']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['nikal_mangani']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['nikal_mangani_by_7']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_difficulty_8']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_application_8']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['phone_verification_8_1']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['phone_verification_option_8_1']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['binkheti_satisfied']);
            $col++;
        ...
        $filename = 'binkheti_report_output_' . date('Ymd_His') . '.xlsx';
...
<div class="col-md-2 mb-3">
    ...
    if ($result->num_rows > 0) {
        ...
        <tr>
            ...
            <th>binkheti Application 2</th>
            <th>binkheti Application 3</th>
            <th>binkheti Application 3.1</th>
            <th>binkheti Difficulty 4</th>
            <th>binkheti Multiple Selection 4</th>
            <th>binkheti Application 5</th>
            <th>binkheti Application Reason 5</th>
            <th>binkheti Application Response 6</th>
            <th>binkheti Office Selection 6</th>
            <th>Nikal Mangani</th>
            <th>Nikal Mangani By 7</th>
            <th>binkheti Difficulty 8</th>
            <th>binkheti Application 8</th>
     ...
     <td>";
        ...
        Your browser does not support the audio element.
            ...
            echo "</td>
                ...
                <td>{$row['binkheti_relativename']}</td> 
                <td>{$row['applicant_location']}</td>
                <td>{$row['applicant_taluko']}</td>
                <td>{$row['applicant_jilla']}</td>
                <td>{$row['binkheti_application_2']}</td>
                <td>{$row['binkheti_application_3']}</td>
                <td>{$row['binkheti_application_3_1']}</td>
                <td>{$row['binkheti_difficulty_4']}</td>
                <td>{$row['binkheti_multiple_selection_4']}</td>
                <td>{$row['binkheti_application_5']}</td>
                <td>{$row['binkheti_application_reason_5']}</td>
                <td>{$row['binkheti_application_response_6']}</td>
                <td>{$row['binkheti_office_selection_6']}</td>
                <td>{$row['nikal_mangani']}</td>
                <td>{$row['nikal_mangani_by_7']}</td>
                <td>{$row['binkheti_difficulty_8']}</td>
                <td>{$row['binkheti_application_8']}</td>
                <td>{$row['phone_verification_8_1']}</td>
                <td>{$row['phone_verification_option_8_1']}</td>
                <td>{$row['binkheti_satisfied']}</td>
