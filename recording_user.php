...
// Handle user filter
$agent = isset($_GET['agent']) ? $mysqli->real_escape_string($_GET['agent']) : '';
...
// Use static agent value for the user filter
if (!empty($agent)) {
    $totalRecordsQuery .= " AND r.user = '$agent'";
}
...
$sql .= " ORDER BY r.start_time LIMIT $recordsPerPage OFFSET $offset"; // Limit and offset
...
<div class="col-md-3 mb-3">
    ...
    <select name="user_filter" id="user_filter" class="form-select">
        <?php
        // Output a default option
        echo "<option value='1001' selected>1001</option>";
        ?>
    </select>
</div>
                <div class="col-md-3 mb-3">
