...
$sql = "SELECT kfd.*, rl.location AS recording_location, SEC_TO_TIME(vl.length_in_sec) AS duration,
        DATE(vl2.call_date) AS call_date, 
        TIME(vl2.call_date) AS call_time
        FROM khedut_form_data kfd
        INNER JOIN recording_log rl ON kfd.rec_id = rl.recording_id
        INNER JOIN vicidial_log vl ON kfd.phonenumber = vl.phone_number
        INNER JOIN vicidial_log vl2 ON kfd.lead_id = vl2.lead_id
        WHERE (`agent` LIKE '%$agentFilter%')
          ...
          AND (
            `khedut_application_2` LIKE '%%$answerFilter%%' OR `khedut_application_2` LIKE '%%$answerFilter%%'
  )
  AND (
    CONCAT_WS(' ', `agent`, `phonenumber`, `application_no`, `application_date`, `disposal_date`, `applicant_name`, `khedut_relativename`, `applicant_location`, `applicant_taluko`, `applicant_jilla`, `star_rating`, 
    `khedut_application_2`, `khedut_application_3`, `khedut_application_3_1`, `khedut_application_3_2`, `khedut_application_fee_3_2`, 
    `khedut_difficulty_4`, `khedut_multiple_selection_4`, `khedut_application_5`, `khedut_application_reason_5`, 
    `khedut_application_response_6`, `khedut_office_selection_6`, `khedut_nikal_mangani`, `khedut_nikal_mangani_by_7`, `khedut_kharai_praman`, 
    `khedut_kharai_praman_by_8`, `suggestion`)
    LIKE '%$wildcardFilter%'
)
  GROUP BY kfd.id desc";
...
if (isset($_GET['export']) && $_GET['export'] == 'xlsx') {
    ...
    if ($result->num_rows > 0) {
        ...
        $headers = [
            ...
            'Applicant Taluko',
            'Applicant Jilla',
            'Khedut Application 2',
            'Khedut Application 3',
            'Khedut Application 3.1',
            'Khedut Difficulty 4',
            'Khedut Multiple Selection 4',
            'Khedut Application 5',
            'Khedut Application Reason 5',
            'Khedut Application Response 6',
            'Khedut Office Selection 6',
            'Nikal Mangani',
            'Nikal Mangani By 7',
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_relativename']);
            $col++;
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_2']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_3']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_3_1']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_fee_3_2']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_difficulty_4']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_multiple_selection_4']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_5']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_reason_5']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_application_response_6']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_office_selection_6']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_nikal_mangani']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_nikal_mangani_by_7']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_service_confirmation']); 
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['khedut_satisfied']); 
            $col++;
        ...
        $filename = 'khedut_report_output_' . date('Ymd_His') . '.xlsx';
 ...
 // Display data in HTML table
    if ($result->num_rows > 0) {
        ...
        <tr>
            ...
            <th>Khedut Application 2</th>
            <th>Khedut Application 3</th>
            <th>Khedut Application 3.1</th>
            <th>Khedut Application Fee 3.2</th>
            <th>Khedut Difficulty 4</th>
            <th>Khedut Multiple Selection 4</th>
            <th>Khedut Application 5</th>
            <th>Khedut Application Reason 5</th>
            <th>Khedut Application Response 6</th>
            <th>Khedut Office Selection 6</th>
            <th>Nikal Mangani</th>
  ...
  $index = 1; // Initialize index
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            echo "</td>
                ...
                <td>{$row['khedut_relativename']}</td> 
                <td>{$row['applicant_location']}</td>
                <td>{$row['applicant_taluko']}</td>
                <td>{$row['applicant_jilla']}</td>
                <td>{$row['khedut_application_2']}</td>
                <td>{$row['khedut_application_3']}</td>
                <td>{$row['khedut_application_3_1']}</td>
                <td>{$row['khedut_application_fee_3_2']}</td>
                <td>{$row['khedut_difficulty_4']}</td>
                <td>{$row['khedut_multiple_selection_4']}</td>
                <td>{$row['khedut_application_5']}</td>
                <td>{$row['khedut_application_reason_5']}</td>
                <td>{$row['khedut_application_response_6']}</td>
                <td>{$row['khedut_office_selection_6']}</td>
                <td>{$row['khedut_nikal_mangani']}</td>
                <td>{$row['khedut_nikal_mangani_by_7']}</td>
                <td>{$row['khedut_service_confirmation']}</td>
                <td>{$row['khedut_satisfied']}</td>
