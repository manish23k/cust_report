...
// $filename = 'hayati_report_output_' . date('Ymd_His') . '.xlsx';
...
$sql = "SELECT hfd.*, rl.location AS recording_location, SEC_TO_TIME(vl.length_in_sec) AS duration,
        DATE(vl2.call_date) AS call_date, 
        TIME(vl2.call_date) AS call_time
        FROM hayati_form_data hfd
        INNER JOIN recording_log rl ON hfd.rec_id = rl.recording_id
        INNER JOIN vicidial_log vl ON hfd.phonenumber = vl.phone_number
        INNER JOIN vicidial_log vl2 ON hfd.lead_id = vl2.lead_id
        WHERE (`agent` LIKE '%$agentFilter%')
           ...
           AND (
            `hayati_application_2` LIKE '%%$answerFilter%%' OR `hayati_application_2` LIKE '%%$answerFilter%%'
   )
   AND (
    CONCAT_WS(' ', `agent`, `phonenumber`, `application_no`, `application_date`, `disposal_date`, `applicant_name`,`hayati_relativename`, `applicant_location`, `applicant_taluko`, `applicant_jilla`, `star_rating`, 
    `star_rating`, `hayati_application_2`, `hayati_application_3`, `hayati_application_3_1`, `hayati_difficulty_4`, 
    `hayati_multiple_selection_4`, `hayati_application_5`, `hayati_application_reason_5`, 
    `hayati_application_response_6`, `hayati_office_selection_6`, `nikal_mangani`, 
    `nikal_mangani_by_7`, `service_confirmation`, `suggestion`)
    LIKE '%$wildcardFilter%'
)
   GROUP BY hfd.id desc";
//echo "$sql";
...
if (isset($_GET['export']) && $_GET['export'] == 'xlsx') {
    ...
    if ($result->num_rows > 0) {
        ...
        $headers = [
            ...
            'Applicant Taluko',
            'Applicant Jilla',
            'Hayati Application 2',
            'Hayati Application 3',
            'Hayati Application 3.1',
            'Hayati Difficulty 4',
            'Hayati Multiple Selection 4',
            'Hayati Application 5',
            'Hayati Application Reason 5',
            'Hayati Application Response 6',
            'Hayati Office Selection 6',
            'Nikal Mangani',
            'Nikal Mangani By 7',
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_relativename']);
            $col++;
            ...
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_2']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_3']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_3_1']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_difficulty_4']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_multiple_selection_4']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_5']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_reason_5']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_application_response_6']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_office_selection_6']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['nikal_mangani']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['nikal_mangani_by_7']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['service_confirmation']);
            $col++;
            $sheet->setCellValue($col . $rowIndex, $row['hayati_satisfied']);
            $col++;
...
<body>
    <h2>હયાતીમા હક્કદાખાલ રિપોર્ટ</h2>
  ...
  // Display data in HTML table
    if ($result->num_rows > 0) {
        ...
        <tr>
            ...
            <th>Hayati Application 2</th>
            <th>Hayati Application 3</th>
            <th>Hayati Application 3.1</th>
            <th>Hayati Difficulty 4</th>
            <th>Hayati Multiple Selection 4</th>
            <th>Hayati Application 5</th>
            <th>Hayati Application Reason 5</th>
            <th>Hayati Application Response 6</th>
            <th>Hayati Office Selection 6</th>
            <th>Nikal Mangani</th>
     ...
     <td>";
        ...
        Your browser does not support the audio element.
            ...
            echo "</td>
                ...
                <td>{$row['hayati_relativename']}</td> 
                <td>{$row['applicant_location']}</td>
                <td>{$row['applicant_taluko']}</td>
                <td>{$row['applicant_jilla']}</td>
                <td>{$row['hayati_application_2']}</td>
                <td>{$row['hayati_application_3']}</td>
                <td>{$row['hayati_application_3_1']}</td>
                <td>{$row['hayati_difficulty_4']}</td>
                <td>{$row['hayati_multiple_selection_4']}</td>
                <td>{$row['hayati_application_5']}</td>
                <td>{$row['hayati_application_reason_5']}</td>
                <td>{$row['hayati_application_response_6']}</td>
                <td>{$row['hayati_office_selection_6']}</td>
                <td>{$row['nikal_mangani']}</td>
                <td>{$row['nikal_mangani_by_7']}</td>
                <td>{$row['service_confirmation']}</td>
                 <td>{$row['hayati_satisfied']}</td>
