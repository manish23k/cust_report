...
// Function to calculate the start and end dates for the given time period
function calculateDateRange($timePeriod)
{
    ...
    if ($timePeriod === 'specific_time') {
        // For specific time, set the start and end dates to today's date and time
        $startDate = clone $currentDateTime;
        $endDate = $currentDateTime;
    } elseif ($timePeriod === 'daily') {
        // For daily, set the start date to today and the end date to tomorrow
        $startDate = clone $currentDateTime;
        $endDate = clone $currentDateTime;
        $endDate->modify('+1 day');
    } elseif ($timePeriod === 'weekly') {
        // For weekly, set the start date to 7 days ago and the end date to today
        $startDate = clone $currentDateTime;
        $startDate->modify('-7 days');
        $endDate = clone $currentDateTime;
    } elseif ($timePeriod === 'monthly') {
        // For monthly, set the start date to the first day of the current month
        $startDate = clone $currentDateTime;
    ...
    return [
        'call_date_from' => $startDate->format('Y-m-d H:i:s'),
        'call_date_to' => $endDate->format('Y-m-d H:i:s'),
    ];
}

// Set the desired time period ('specific_time', 'daily', 'weekly', 'monthly')
$timePeriod = 'weekly';

// Fetch date range based on the time period
...
<head>
    ...
    function toggleHelpVisibility() {
        ...
        helpText.style.display = helpText.style.display === 'none' ? 'inline' : 'none';
    }
</script>
...
<body>
    <h2>Email Report Configuration</h2>
    ...
    <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
        ...
        <label for="report_link">Report Link:</label>
<input type="text" name="report_link" required value="<?php echo isset($editReport['report_link']) ? $editReport['report_link'] : ''; ?>" title="Mouse over for help" onmouseover="showHelp()" onmouseout="hideHelp()" onclick="toggleHelpVisibility()">
<span id="helpText" style="font-size: 12px; color: #007bff; display: none;">(Use Report Search Genrated Link and Add "&export=xlsx" at last)</span><br> 



        <!-- Add a field for email_type -->
