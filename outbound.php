...
$recordsPerPage = 500; // Number of records to display per page
...
$inboundGroupsQuery = "SELECT campaign_id, campaign_name FROM vicidial_campaigns";
...
$statusQuery = "SELECT DISTINCT status FROM vicidial_log";
...
$query = "SELECT list_id, campaign_id, call_date, length_in_sec, status, phone_number, user, term_reason FROM vicidial_log WHERE call_date BETWEEN '$fromDate $fromTime' AND '$toDate $toTime'";
...
// Order by call_date in descending order (latest entries first)
...
$totalCallsQuery = "SELECT COUNT(*) as total FROM vicidial_log WHERE call_date BETWEEN '$fromDate $fromTime' AND '$toDate $toTime'";
...
<body>    
    <div class="container mt-4 text-center">
        <h1 class="display-4">Outbound Call Log</h1>


        <form method="get" action="" class="mb-4">
            <div class="row">
                <div class="col-md-3 mb-3">
                    <label for="campaign">Outbound Group:</label>
    ...
    <div class="col-md-6">
        <?php if ($page > 1): ?>
            <a href="?page=<?= ($page - 1) ?>&campaign=<?= $campaign ?>&from_date=<?= $fromDate ?>&to_date=<?= $toDate ?>&status=<?= $statusFilter ?>&phone_number=<?= $phoneFilter ?>&user=<?= $userFilter ?>"
    ...
    <script>
        function downloadCSV() {
            ...
            <?php
            // Fetch data for CSV
            $csvQuery = "SELECT list_id, campaign_id, call_date, length_in_sec, status, phone_number, user, term_reason FROM vicidial_log WHERE DATE(call_date) BETWEEN '$fromDate $fromTime' AND '$toDate $toTime'";
            ...
            link.setAttribute("download", "outbound_call_log_" + <?php echo json_encode($currentDateTime); ?> + ".csv");
