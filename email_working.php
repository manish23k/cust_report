...
function sendScheduledEmails()
{
    ...
    try {
        ...
        foreach ($reportConfigs as $reportConfig) {
            ...
            // Add these lines to check if the script reaches this point
            echo "Before switch\n";
            ...
            switch ($emailTimeType) {
                case EMAIL_TYPE_DAILY:
                    ...
                    $sendEmail = isTimeToSend($specificTime, $reportConfig['last_sent'], 'daily');
                    break;
                case EMAIL_TYPE_WEEKLY:
                    echo "Inside case EMAIL_TYPE_WEEKLY\n";
                    $sendEmail = isTimeToSend($specificTime, $reportConfig['last_sent'], 'weekly');
                    break;
                case EMAIL_TYPE_MONTHLY:
                    echo "Inside case EMAIL_TYPE_MONTHLY\n";
                    $sendEmail = isTimeToSend($specificTime, $reportConfig['last_sent'], 'monthly');
                    break;
                case EMAIL_TYPE_SPECIFIC_TIME:
                    echo "Inside case EMAIL_TYPE_SPECIFIC_TIME\n";
                    $sendEmail = isTimeToSend($specificTime, $reportConfig['last_sent'], 'specific_time');
                    break;
                default:
...
// Function to check if it's time to send the email for different types
function isTimeToSend($specificTime, $lastSent, $type)
{
    echo "Inside isTimeToSend function\n";
    echo "Specific Time: $specificTime\n";
    echo "Last Sent Time: $lastSent\n";
    echo "Type: $type\n";
    ...
    if (!empty($specificTime)) {
        ...
        switch ($type) {
            case 'daily':
                echo "Inside case 'daily' condition check\n";
                echo "Current Time: " . date('Y-m-d H:i:s', time()) . "\n";
                echo "Scheduled Time: " . date('Y-m-d H:i:s', $scheduledTime) . "\n";
                echo "Last Sent Time: " . date('Y-m-d H:i:s', $lastSentTime) . "\n";
            
                // Compare the current date with the date of the last sent time
                $dateCheck = date('Y-m-d') != date('Y-m-d', strtotime($lastSentTime));
            
                // Check if the current time is greater than or equal to the scheduled time
                $timeCheck = time() >= $scheduledTime;
            
                echo "Date Check: " . ($dateCheck ? 'true' : 'false') . "\n";
                echo "Time Check: " . ($timeCheck ? 'true' : 'false') . "\n";
            
                $result = $timeCheck && $dateCheck;
                break;
            case 'weekly':
                $result = (time() >= $scheduledTime && date('W', $lastSentTime) != date('W'));
                break;
            case 'monthly':
                $result = (time() >= $scheduledTime && date('m', $lastSentTime) != date('m'));
                break;
            case 'specific_time':
                $result = (time() >= $scheduledTime);
                break;
            default:
                $result = false;
        }

        echo "Type: $type, Result: " . ($result ? 'true' : 'false') . "\n";
